                        ...
                        if ($this->db) {
                            ...
                            if ($user) {
                                if($user['SUPER_ADMIN'] == 1) {
                                    ...
                                    $shipmentSql = "SELECT cor_shipments.*, MOBILE_NUMBER, EMAIL_ADDRESS, FULL_NAME FROM cor_shipments JOIN ptf_user_details ON ptf_user_details.USER_ID = cor_shipments.CREATOR_ID WHERE SHIPMENT_STATUS IN ('APPROVED', 'PENDING') AND (SHIPMENT_NUMBER = '' OR SHIPMENT_NUMBER IS NULL) ORDER BY cor_shipments.SHIPMENT_CATEGORY ASC, cor_shipments.CREATED_DATE ASC";
                                    $shipmentStatement = $this->db->prepare($shipmentSql);
                                } else {
								...
								{									
									$tempx[] = array_map('utf8_encode', $shipment);
								}
								$shipments = $tempx;
                                if(!empty($shipments)) {
                                    foreach ($shipments as $key => $shipment) {
