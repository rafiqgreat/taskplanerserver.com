...
class TaskController extends ControllerBase {
	 ...
	 */
	public
	function fetchProjectTasks( $request, $response ) {
		try {
	 ...
	 */
	public
	function createTasks( $request, $response, $arg ) {
		try {
	...
	//createProjTasks
	public
	function createProjTasks( $request, $response, $arg ) {
		try {
			$params = $request->getParsedBody();
			if ( !empty( $params ) ) {
				//if (AuthValidatorController::validateAuth($this->db, $params[ParamKeys::AUTH_TOKEN], $params[ParamKeys::SESSION_ID])) {
				if ( $params[ ParamKeys::TASK_TITLE ] == "" || $params[ ParamKeys::CREATOR_ID ] == "" || $params[ ParamKeys::PROJECT_ID ] == "" || $params[ ParamKeys::ASSIGNED_ID ] == "" ) {
					return $this->sendHTTPResponseError( $response, Messages::MSG_ERR_PARAMS_VALUE_MISSING );
				} else {
					if ( $this->db ) {
						...
						//echo $ASSIGNED_ID . '<br />';
						$ASSIGNED_ID = '%' . substr( $ASSIGNED_ID, -9 );
						//echo $ASSIGNED_ID . '<br />';
						...
						if ( $taskDuplicate ) {
							...
							$statementTask->bindParam( ":taskDescription", $params[ ParamKeys::TASK_DESCRIPTION ], \PDO::PARAM_STR );
							$statementTask->bindParam( ":creatorId", $params[ ParamKeys::CREATOR_ID ], \PDO::PARAM_STR );
							$statementTask->bindParam( ":assignedId", $ASSIGNED_ID, \PDO::PARAM_STR );
							$statementTask->bindParam( ":createdDate", $params[ ParamKeys::CREATED_DATE ], \PDO::PARAM_STR );
							$statementTask->bindParam( ":dueDate", $params[ ParamKeys::DUE_DATE ], \PDO::PARAM_STR );
	 ...
	 */
	public
	function createTasksWeb( $request, $response, $arg ) {
		try {
			$params = $request->getParsedBody();
			if ( !empty( $params ) ) {
				//if (AuthValidatorController::validateAuth($this->db, $params[ParamKeys::AUTH_TOKEN], $params[ParamKeys::SESSION_ID])) {
	 ...
	 */
	public
	function createPersonalTask( $request, $response, $arg ) {
		try {
			$params = $request->getParsedBody();
			if ( !$params )
				...
				if ( $params[ ParamKeys::TASK_TITLE ] == "" || $params[ ParamKeys::CREATOR_ID ] == "" || $params[ ParamKeys::ASSIGNED_ID ] == "" ) {
					...
					if ( $this->db ) {
						...
						if ( $taskDuplicate ) {
							...
							$crdate = time();
							//$duedatet = @date("Y-m-d H:i:s",strtotime($params[ParamKeys::DUE_DATE_DT]));
							$statementTask->bindParam( ":createdDate", $crdate );


							//die('DueDate:'.$params[ParamKeys::DUE_DATE_DT].'=dt:'.$duedatet);


							//die($params[ParamKeys::REPEAT_INTERVAL]);

							$statementTask->bindParam( ":dueDate", $params[ ParamKeys::DUE_DATE ], \PDO::PARAM_STR );
							$statementTask->bindParam( ":dueDateDt", $params[ ParamKeys::DUE_DATE_DT ], \PDO::PARAM_STR );
	 ...
	 */
	public
	function updateTaskStatus( $request, $response ) {
		try {
			$params = $request->getParsedBody();
			if ( !empty( $params ) ) {
				//if (AuthValidatorController::validateAuth($this->db, $params[ParamKeys::AUTH_TOKEN], $params[ParamKeys::SESSION_ID])) {
...
$dt = DateTime::createFromFormat('Y-m-d  H:i:s', $nextDueDate);
	...
	public
	function changeProjectTaskDetails( $request, $response ) {
		try {
			//$params = $request->getParsedBody();
			$params = $request->getParams();
			if ( !empty( $params ) ) {
				...
				if ( $params[ ParamKeys::TASK_ID ] == "" ) {
					...
					if ( $this->db ) {
						$this->db->beginTransaction();
						$tasksStatement = $this->db->prepare( 'UPDATE ' . $this->getTasks() . ' set TASK_TITLE = :taskTitle, TASK_DESCRIPTION = :taskDesc, DUE_DATE = :dueDate, TASK_STATUS = :status where TASK_ID = :taskId' );
						$tasksStatement->bindParam( ":taskTitle", $params[ ParamKeys::TASK_TITLE ], \PDO::PARAM_STR );
						$tasksStatement->bindParam( ":taskDesc", $params[ ParamKeys::TASK_DESCRIPTION ], \PDO::PARAM_STR );
	 ...
	 */
	public
	function assignTask( $request, $response ) {
		try {
			$params = $request->getParsedBody();
			if ( !empty( $params ) ) {
				//if (AuthValidatorController::validateAuth($this->db, $params[ParamKeys::AUTH_TOKEN], $params[ParamKeys::SESSION_ID])) {
	 ...
	 */
	public
	function fetchUserTasks( $request, $response ) {
		try {
			$params = $request->getQueryParams();
			if ( !empty( $params ) ) {
				//if (AuthValidatorController::validateAuth($this->db, $params[ParamKeys::AUTH_TOKEN], $params[ParamKeys::SESSION_ID])) {
...
public
 ...
 FROM cor_project_tasks task where task.CREATOR_ID = :userId AND task.ASSIGNED_ID = :userId AND task.PROJECT_ID IS NULL";
	 ...
	 */
	public
	function fetchTasks( $request, $response ) {
		try {
			$params = $request->getQueryParams();
			if ( !empty( $params ) ) {
				//if (AuthValidatorController::validateAuth($this->db, $params[ParamKeys::AUTH_TOKEN], $params[ParamKeys::SESSION_ID])) {
	 ...
	 */
	public
	function fetchOthersTasks( $request, $response ) {
		try {
			$params = $request->getQueryParams();
			if ( !empty( $params ) ) {
				//if (AuthValidatorController::validateAuth($this->db, $params[ParamKeys::AUTH_TOKEN], $params[ParamKeys::SESSION_ID])) {
	...
	public
	function fetchOthersTasksDD( $request, $response ) {
		try {
			$params = $request->getQueryParams();
			if ( !empty( $params ) ) {
				//if (AuthValidatorController::validateAuth($this->db, $params[ParamKeys::AUTH_TOKEN], $params[ParamKeys::SESSION_ID])) {
	 ...
	 */
	public
	function fetchMeTasks( $request, $response ) {
		try {
			$params = $request->getQueryParams();
			if ( !empty( $params ) ) {
				//if (AuthValidatorController::validateAuth($this->db, $params[ParamKeys::AUTH_TOKEN], $params[ParamKeys::SESSION_ID])) {
	...
	public
	function fetchMemberDetails( $request, $response ) {
		try {
			$params = $request->getQueryParams();
			if ( !empty( $params ) ) {
				//if (AuthValidatorController::validateAuth($this->db, $params[ParamKeys::AUTH_TOKEN], $params[ParamKeys::SESSION_ID])) {
	 ...
	 */
	public
	function fetchStatusTasks( $request, $response ) {
		try {
			$params = $request->getQueryParams();
			if ( !empty( $params ) ) {
				//if (AuthValidatorController::validateAuth($this->db, $params[ParamKeys::AUTH_TOKEN], $params[ParamKeys::SESSION_ID])) {
	 ...
	 */
	public
	function fetchPersonalTasks( $request, $response ) {
		try {
			$params = $request->getQueryParams();
			if ( !empty( $params ) ) {
				//if (AuthValidatorController::validateAuth($this->db, $params[ParamKeys::AUTH_TOKEN], $params[ParamKeys::SESSION_ID])) {
	...
	public
	function fetchOnlyPersonalTasks( $request, $response ) {
		try {
			$params = $request->getQueryParams();
			if ( !empty( $params ) ) {
				//if (AuthValidatorController::validateAuth($this->db, $params[ParamKeys::AUTH_TOKEN], $params[ParamKeys::SESSION_ID])) {
	...
	private
	function getTaskNotificationFromNocTable( $userid, $taskid ) {
	 ...
	 */
	public
	function fetchSubTasks( $request, $response ) {
		try {
			$params = $request->getQueryParams();
			if ( !empty( $params ) ) {
				//if (AuthValidatorController::validateAuth($this->db, $params[ParamKeys::AUTH_TOKEN], $params[ParamKeys::SESSION_ID])) {
				if ( $params[ ParamKeys::TASK_ID ] == "" ) {
					...
					if ( $this->db ) {
						$taskSql = "select * from " . $this->getTasks() . " LEFT JOIN ptf_user_details ON ptf_user_details.USER_ID = cor_project_tasks.ASSIGNED_ID where PARENT_TASK_ID = :taskId";
						//JOIN ptf_user_details ON ptf_user_details.USER_ID = cor_shipments.CREATOR_ID
						$tasksStatement = $this->db->prepare( $taskSql );
						$tasksStatement->bindParam( ":taskId", $params[ ParamKeys::TASK_ID ], \PDO::PARAM_STR );
						$tasksStatement->execute();
	 ...
	 */
	public
	function fetchCCTasksUsers( $request, $response ) {
		try {
			$params = $request->getQueryParams();
			if ( !empty( $params ) ) {
				//if (AuthValidatorController::validateAuth($this->db, $params[ParamKeys::AUTH_TOKEN], $params[ParamKeys::SESSION_ID])) {
	...
	public
	function fetchCCDueTasksUsers( $request, $response ) {
		try {
			$params = $request->getQueryParams();
			if ( !empty( $params ) ) {
				//if (AuthValidatorController::validateAuth($this->db, $params[ParamKeys::AUTH_TOKEN], $params[ParamKeys::SESSION_ID])) {
	...
	public
	function fetchCCDueTasksInterval( $request, $response ) {
		try {
			$params = $request->getQueryParams();
			if ( !empty( $params ) ) {
				//if (AuthValidatorController::validateAuth($this->db, $params[ParamKeys::AUTH_TOKEN], $params[ParamKeys::SESSION_ID])) {
	...
	public
	function fetchCCDueTasks( $request, $response ) {
		try {
			$params = $request->getQueryParams();
			if ( !empty( $params ) ) {
				//if (AuthValidatorController::validateAuth($this->db, $params[ParamKeys::AUTH_TOKEN], $params[ParamKeys::SESSION_ID])) {
	...
	public
	function fetchCCTasksInterval( $request, $response ) {
	...
	// Orignal Class
	public
	function fetchCCTasks( $request, $response ) {
		try {
			$params = $request->getQueryParams();
			if ( !empty( $params ) ) {
				//if (AuthValidatorController::validateAuth($this->db, $params[ParamKeys::AUTH_TOKEN], $params[ParamKeys::SESSION_ID])) {
	...
	public
	function countTasksDD( $request, $response ) {
		try {
			// $params = $request->getQueryParams();
			$params = $request->getParams();
			if ( !empty( $params ) ) {
	...
	public
	function countTasks( $request, $response ) {
		try {
			$params = $request->getQueryParams();
			if ( !empty( $params ) ) {
				//if (AuthValidatorController::validateAuth($this->db, $params[ParamKeys::AUTH_TOKEN], $params[ParamKeys::SESSION_ID])) {
	...
	public
	function fetchCCTasksOnly( $request, $response ) {
		try {
			$params = $request->getQueryParams();
			if ( !empty( $params ) ) {
				//if (AuthValidatorController::validateAuth($this->db, $params[ParamKeys::AUTH_TOKEN], $params[ParamKeys::SESSION_ID])) {
	...
	public
	function fetchOnlyCCTasksDD( $request, $response ) {
		try {
			$params = $request->getQueryParams();
			if ( !empty( $params ) ) {
				//if (AuthValidatorController::validateAuth($this->db, $params[ParamKeys::AUTH_TOKEN], $params[ParamKeys::SESSION_ID])) {
	 ...
	 */
	public
	function removeFromCCTask( $request, $response ) {
		try {
			$params = $request->getParsedBody();
			if ( !empty( $params ) ) {
				//if (AuthValidatorController::validateAuth($this->db, $params[ParamKeys::AUTH_TOKEN], $params[ParamKeys::SESSION_ID])) {
	 ...
	 */
	public
	function uploadTaskImages( $request, $response ) {
		try {
			$params = $request->getParsedBody();
			if ( !empty( $params ) ) {
				//if (AuthValidatorController::validateAuth($this->db, $params[ParamKeys::AUTH_TOKEN], $params[ParamKeys::SESSION_ID])) {
	 ...
	 */
	private
	function _get_project_tasks( $projectId ) {
	 ...
	 */
	private
	function _get_personal_tasks( $creatorId ) {
	 ...
	 */
	public
	function fetchNotifications( $request, $response ) {
		try {
			$params = $request->getQueryParams();
			if ( !empty( $params ) ) {
				//if (AuthValidatorController::validateAuth($this->db, $params[ParamKeys::AUTH_TOKEN], $params[ParamKeys::SESSION_ID])) {
	 ...
	 */
	public
	function fetchPopupNotifications( $request, $response ) {
		try {
			$params = $request->getQueryParams();
			if ( !empty( $params ) ) {
				//if (AuthValidatorController::validateAuth($this->db, $params[ParamKeys::AUTH_TOKEN], $params[ParamKeys::SESSION_ID])) {
				if ( $params[ ParamKeys::USER_ID ] == "" ) {
					...
					/*
					$temp = [];
					$temp['title'] = 'Task Planner';
					$temp['msg'] = 'New Notification';
					$temp['icon'] = 'http://localhost/www/taskplannerserver.com/assets/images/add-ico-20.png';
					$temp['url'] = 'http://localhost/www/taskplannerserver.com/';
					$notifications = array("notif"=>array($temp),"count"=>2,"result"=>true);
					return $this->sendHttpResponseSuccess($response, $notifications);
					*/
					...
					if ( $this->db ) {
						...
						if ( $tasks ) {
							...
							foreach ( $tasks as $task ) {
								...
								//$temp['url'] = 'javascript:void();';
								$tempx[] = $temp;
							}
							$notifications = array( "notif" => $tempx, "count" => count( $tempx ), "result" => true );
							return $this->sendHttpResponseSuccess( $response, $notifications );
						} else {
							return $this->sendHTTPResponseError( $response, Messages::MSG_ERR_PROJECT_TASK_FETCH );
						}
	 ...
	 */
	public
	function fetchProjectDueTasks( $request, $response ) {
		try {
			$params = $request->getQueryParams();
			if ( !empty( $params ) ) {
				//if (AuthValidatorController::validateAuth($this->db, $params[ParamKeys::AUTH_TOKEN], $params[ParamKeys::SESSION_ID])) {
	...
	public
	function taskNotificationTest( $request, $response ) {
		try {
			$params = $request->getParsedBody();
			if ( !empty( $params ) ) {
				//if (AuthValidatorController::validateAuth($this->db, $params[ParamKeys::AUTH_TOKEN], $params[ParamKeys::SESSION_ID])) {
	...
	public
	function changeTaskNotificationStatus( $request, $response ) {
	...
	public
	function createQbGroup( $request, $response ) {
		try {
			$params = $request->getParsedBody();
			if ( !empty( $params ) ) {
				//if (AuthValidatorController::validateAuth($this->db, $params[ParamKeys::AUTH_TOKEN], $params[ParamKeys::SESSION_ID])) {
	...
	private
	function getUsersFromCC( $user_ids ) {
	...
	private
	function addTasksImages( $filename, $taskId ) {
		if ( !is_null( $filename ) ) {
			$this->db->beginTransaction();
	...
	public
	function advSearchTasks( $request, $response ) {
			...
			if ( !empty( $params ) ) {
				...
				if ( $params[ ParamKeys::USER_ID ] == "" ) {
					...
					if ( $this->db ) {	
						...
						//////////////////////////////////
						$sTasks = [];
						$sTasks[ 'SUPER_ADMIN' ] 	= $user[ 'SUPER_ADMIN' ];
						$sTasks[ 'IS_SHIPPER' ] 	= $user[ 'IS_SHIPPER' ];
						$sTasks[ 'SHIPMENTS' ] 		= $shipments;
						$sTasks[ 'NPTASKS' ] 		= $npTasks;
						$sTasks[ 'PROJTASKS' ] 		= $pTasks;

						return $this->sendHttpResponseSuccess( $response, $sTasks );
