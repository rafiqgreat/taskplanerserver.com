...
class TaskController extends ControllerBase
{
  ...
  //createProjTasks
      public function createProjTasks($request, $response, $arg) {
        try {
            $params = $request->getParsedBody();
            if (!empty($params) ) {
                //if (AuthValidatorController::validateAuth($this->db, $params[ParamKeys::AUTH_TOKEN], $params[ParamKeys::SESSION_ID])) {
...
//            $this->db->rollBack();
  ...
  public function changeProjectTaskDetails($request, $response) {
        try {
            //$params = $request->getParsedBody();
			$params = $request->getParams(); 
            if (!empty($params) ) {
...
//die($datenew);
  ...
  /**
     * Description: fetch all notifications on basis of userID
     */
    public function fetchPopupNotifications($request, $response) {
        try {
            $params = $request->getQueryParams();
            if (!empty($params) ) {				
                //if (AuthValidatorController::validateAuth($this->db, $params[ParamKeys::AUTH_TOKEN], $params[ParamKeys::SESSION_ID])) {
