            ...
            if (!empty($params) ) {
                //if (AuthValidatorController::validateAuth($this->db, $params[ParamKeys::AUTH_TOKEN], $params[ParamKeys::SESSION_ID])) {
                    if ($params[ParamKeys::USER_ID] == "") {
                        ...
                        if ($this->db) {
                            $taskSql = "SELECT " .  $this->getTasks() . ".*, MOBILE_NUMBER, EMAIL_ADDRESS, FULL_NAME  FROM ". $this->getTasks() . " 
                                        JOIN " . $this->getUserDetails() . " ON " . $this->getUserDetails() . ".USER_ID = ". $this->getTasks() . ".ASSIGNED_ID
